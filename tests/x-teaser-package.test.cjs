...
test("Chrome native package is versioned from the personal manifest and installation stays opt-in", () => {
  ...
  const build = fs.readFileSync(
    path.join(root, "scripts", "build-x-teaser-native-host.ps1"),
    "utf8",
  );
...
test("Chrome-only setup documents all separately authorized live gates", () => {
  const smoke = fs.readFileSync(
    path.join(root, "docs", "X_TEASER_AUTHENTICATED_SMOKE.md"),
    "utf8",
  );
  assert.match(smoke, /Do not press X's final \*\*Post\*\*/);
  assert.match(smoke, /live Work Sheet/);
  assert.match(smoke, /real media roots/);
  assert.doesNotMatch(smoke, /Firefox/i);
});
