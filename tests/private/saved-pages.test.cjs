...
async function testPostSnapshot(page) {
  ...
  const view = await page.evaluate(() => ({
    commentNames: [
      ...document.querySelectorAll(".b-comments__item .g-user-name"),
    ]
    ...
    ownName: document
      .querySelector('[at-attr="my_profile"] .g-user-name')
      ?.textContent.trim(),
  }));
  ...
  assert.equal(
    view.ownName,
    "Johnny Guides",
    "The creator identity must remain unchanged.",
  );
  ...
  assert.ok(
    clickResult.eventDetail?.primaryKey,
    "Avatar clicks must request a reroll.",
  );
  assert.equal(
    clickResult.rerollEvents,
    1,
    "One pointer sequence must reroll only once.",
  );
  assert.equal(
  ...
  assert.equal(
    clickResult.avatarHref,
    null,
    "Masked avatars must not retain profile links.",
  );
  assert.equal(
    clickResult.loading,
    true,
    "Avatar rerolls must show a loading state.",
  );
  assert.notEqual(
    clickResult.radius,
    "0px",
    "Every masked avatar must be circular.",
  );
  ...
  const fallbackSource = await page.evaluate(() => {
    ...
    const image = context.container.querySelector(
      "img[data-fim-masked-avatar]",
    );
...
async function testDmSnapshot(page) {
  ...
  assert.ok(
    result.contextCounts["chat-list"] >= 8,
    "Expected DM conversation rows.",
  );
  assert.equal(
    result.contextCounts["chat-header"],
    1,
    "Expected one active DM header.",
  );
  assert.ok(
  ...
  const view = await page.evaluate(() => ({
    selectedListName: document
      .querySelector(".b-chats__item.current [at-attr='chat_list_user_name']")
      ?.textContent.trim(),
    selectedListHandle: document
      .querySelector(".b-chats__item.current .g-user-username")
      ?.textContent.trim(),
    headerName: document
      .querySelector(".b-chat__header__title .g-user-name .g-user-name")
      ?.textContent.trim(),
    incomingAvatarKeys: [
