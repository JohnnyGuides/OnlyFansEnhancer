...
const gelbooruRulesPath = path.resolve(
  require("../support/paths.cjs").personalRoot,
  "gelbooru-rules.json",
);
const storage = {};
...
const chrome = {
  ...
  scripting: {
    ...
    async executeScript(details) {
      if (details.files) {
        if (details.files.includes("workflows/upload-trace-recorder.js")) {
...
(async () => {
  ...
  assert.equal(sharedAppStatus.result.productVersion, "0.20.57");
  await assert.rejects(
    send({
      type: "OFENHANCER_APP_REQUEST",
  ...
  assert.deepEqual(redgifsTrace.js, ["workflows/upload-trace-recorder.js"]);
