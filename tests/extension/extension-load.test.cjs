...
(async () => {
  ...
  try {
    ...
    assert.match(serializedTrace, /"selectedLabels":\["opaque-\d+"\]/);
    assert.match(serializedTrace, /"toolId":"opaque-\d+"/);
    assert.doesNotMatch(serializedTrace, /"Gay"|"onlyfansAutoSelect"/);
    assert.match(serializedTrace, /"status":"success"/);
    assert.match(serializedTrace, /https:\/\/onlyfans\.com\/posts\/\[id\]/);
