...
function loadRuntime(options = {}) {
  ...
  function compactResult(payload, urlField) {
    ...
    return {
      status: options.local ? "recorded-local" : "updated",
      ...(options.local ? { googleSynced: false } : {}),
...
test("deferred catalogue keeps captured posts local and association never republishes", async () => {
  ...
  assert.equal(published.jobs.x.stage, "result-captured");
  assert.equal(published.jobs.x.submitAttempted, true);
  assert.equal(runtime.sheetCalls.length, 0);
  const submits = runtime.calls.filter((call) =>
    call.startsWith("submit:"),
  ).length;
  ...
  await runtime.orchestrator.associateCatalogue(input.id, plan().catalogue);
  await runtime.orchestrator.associateCatalogue(input.id, plan().catalogue);
  assert.equal(
    runtime.calls.filter((call) => call.startsWith("submit:")).length,
    submits,
  ...
  assert.equal(
    (await runtime.store.load(input.id)).jobs.x.stage,
    "sheet-complete",
  );
  assert.equal((await runtime.store.load(input.id)).plan.catalogue, null);
  await assert.rejects(
    runtime.orchestrator.associateCatalogue(input.id, {
      ...plan().catalogue,
      id: "different",
    }),
    /association/,
...
test("association cannot run adapters for unfinished publication jobs", async () => {
  const runtime = loadRuntime();
  const input = plan({ catalogue: null });
  await runtime.store.create(input);
  await runtime.orchestrator.associateCatalogue(input.id, plan().catalogue);
...
test("local catalogue writes stay terminal and visibly unsynced across resume", async () => {
  const runtime = loadRuntime({ local: true });
  const value = plan({ targets: { x: true, reddit: [] } });
  await runtime.store.create(value);
  const completed = await runtime.orchestrator.startSocialDistribution(
    value.id,
  );
  assert.equal(completed.jobs.x.stage, "sheet-complete");
  assert.equal(completed.jobs.x.googleSynced, false);
  const before = runtime.calls.filter((call) =>
    call.startsWith("submit:"),
  ).length;
  const resumed = await runtime.orchestrator.resumeSocialDistribution(value.id);
  assert.equal(resumed.jobs.x.googleSynced, false);
  assert.equal(
    runtime.calls.filter((call) => call.startsWith("submit:")).length,
    before,
  );
