...
const registryPath = path.join(repositoryRoot, "creator-tools", "registry.js");
const commonPath = path.join(repositoryRoot, "creator-tools", "common.js");
...
test("recorder is integrated into the personal toolkit and excluded from store", () => {
  ...
  vm.runInContext(read("creator-tools/registry.js"), registryContext, {
  ...
  for (const obsoleteFile of ["manifest.json", "recorder.js", "README.md"]) {
    assert.equal(
      fs.existsSync(
        path.join(
          repositoryRoot,
          "diagnostics",
          "upload-trace-recorder",
          obsoleteFile,
        ),
      ),
      false,
  ...
  assert.doesNotMatch(read("store/manifest.json"), /upload-trace-recorder/i);
  assert.doesNotMatch(
    read("scripts/build-store-package.ps1"),
