...
function loadRegistry() {
  ...
  vm.runInContext(read("creator-tools/registry.js"), context, {
    filename: "creator-tools/registry.js",
  });
...
test("personal manifest keeps every integrated creator helper active", () => {
  ...
  assert.match(personalBuild, /apps-script[\\/]catalogue-bridge\.gs/);
...
test("every adapter parses and remains dynamically registered", () => {
  ...
  for (const sharedFile of [
    ...
    assert.doesNotThrow(
      () =>
        new vm.Script(read(`creator-tools/${sharedFile}`), {
          filename: sharedFile,
        }),
    );
  }
...
test("runtime and documentation sources are valid UTF-8 without mojibake", () => {
  const sourceFiles = [
    ...
    "PRIVACY.md",
    "STORE_READINESS.md",
    "docs/CREATOR_TOOLS_ARCHITECTURE.md",
    "docs/AUTHENTICATED_SMOKE_TEST.md",
    ...fs
      .readdirSync(toolsRoot)
      .filter((fileName) => fileName.endsWith(".js"))
      .map((fileName) => `creator-tools/${fileName}`),
  ];
...
test("OnlyFans actions are capped, confirmed, stable-ID based, and manually committed", () => {
  const registry = loadRegistry();
  const follow = read("creator-tools/onlyfans-auto-follow.js");
  const select = read("creator-tools/onlyfans-auto-select.js");
  const shared = read("creator-tools/onlyfans-list-common.js");
...
test("the Chrome Web Store edition remains the narrow identity-mask product", () => {
  const storeManifest = JSON.parse(read("store/manifest.json"));
