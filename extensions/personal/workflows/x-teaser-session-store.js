(() => {
  ...
  function sanitizeInput(record) {
    ...
    key(output.id);
    if (record.sheetAttempted === true) output.sheetAttempted = true;
    for (const field of ["reconciliationSource", "remoteCommit"]) {
      if (!Object.hasOwn(record, field)) continue;
      const value = record[field];
      if (
        !value ||
        value.backend !== "apps-script" ||
        !/^[A-Za-z0-9_-]{10,200}$/.test(value.workbookId) ||
        typeof value.sheetName !== "string" ||
        !value.sheetName ||
        value.sheetName.length > 200
      )
        throw new Error("Invalid remote Sheet evidence.");
      if (field === "reconciliationSource") {
        if (
          !/^https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]{20,}\/exec$/.test(
            value.endpoint,
          )
        )
          throw new Error("Invalid remote Sheet endpoint.");
        output[field] = {
        ...
        if (
          ...
          value.row > 5000 ||
          !value.itemId ||
          !contract.canonicalStatusUrl(value.statusUrl)
        )
          throw new Error("Invalid remote Sheet receipt.");
        output[field] = {
          backend: value.backend,
          workbookId: value.workbookId,
          sheetName: value.sheetName,
          row: value.row,
          itemId: clean(value.itemId, 500),
  ...
  function merge(previous, next) {
    ...
    for (const field of ["reconciliationSource", "remoteCommit"]) {
      if (
        ...
        throw new Error("Frozen remote Sheet evidence cannot be changed.");
    }
