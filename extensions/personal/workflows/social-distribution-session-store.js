(() => {
  ...
  function checkpointPatch(value) {
    ...
    if (Object.hasOwn(value, "googleSynced"))
      output.googleSynced = value.googleSynced === true;
  ...
  async function associateCatalogue(id, value) {
    const association = contract.catalogue(value);
    return enqueue(async () => {
      const previous = await load(id);
      if (!previous) throw new Error("Distribution session was not found.");
      const existing = previous.catalogueAssociation || previous.plan.catalogue;
      if (
        existing &&
        JSON.stringify(existing) !== JSON.stringify(association)
      ) {
        throw new Error(
          "The catalogue association is already fixed for this run.",
        );
      }
      if (existing) return previous;
      const next = { ...previous, catalogueAssociation: association };
      await storage.set({ [key(id)]: next });
      return next;
    });
  }
