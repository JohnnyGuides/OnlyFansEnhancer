(() => {
  ...
  // Missing edition configuration must not enable masking.
  let settings = {
    enabled: false,
    consentAccepted: false,
    ownHandles: [],
    ...globalThis.FanIdentityMaskDefaults,
  };
  let scheduled = false;
  let scanning = false;
  ...
  function finishFailedRotation(keys, message) {
    ...
    for (const image of document.querySelectorAll(
      ...
      image
        ...
        ?.classList.remove("fim-avatar-loading");
  ...
  async function rotateAvatar(detail = {}) {
    ...
    try {
      ...
      for (const [key, cached] of identityCache.entries()) {
        if (cached?.handle === identity.handle)
