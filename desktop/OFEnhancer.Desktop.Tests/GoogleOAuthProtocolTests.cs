...
public sealed class GoogleOAuthProtocolTests
{
    ...
    public async Task DesktopCodeExchangeIncludesSecretOnlyInGooglePost()
    {
        ...
        using HttpClient client = new(handler);
        await GoogleOAuthProtocol.ExchangeCodeAsync(client, ClientId, "code", RedirectUri,
            "verifier", CancellationToken.None, "desktop-secret");
        Assert.AreEqual("desktop-secret", ParseForm(handler.Body)["client_secret"]);
        Assert.AreEqual("https://oauth2.googleapis.com/token", handler.RequestUri!.AbsoluteUri);
        Assert.IsFalse(GoogleOAuthProtocol.CreateStart(ClientId, RedirectUri).AuthorizationUri.Query.Contains("secret"));
    }
