...
{
    [TestMethod]
    public async Task RowMovedBeforeDispatchUsesMetadataBoundWrite()
    {
        using SyncFixture fixture = SyncFixture.Create();
        fixture.Enqueue("ashley", Clock);
        fixture.Google.SetRow(fixture.ItemId("ashley"), 93);
        fixture.Google.BeforeMutation = () => fixture.Google.SetRow(fixture.ItemId("ashley"), 94);
        await fixture.RunSelectedAsync();
        Assert.AreEqual(Range(94), fixture.Google.Mutations.Single().Range);
    }

    [TestMethod]
    public async Task MovedSemanticHeaderBlocksWrite()
    {
        using SyncFixture fixture = SyncFixture.Create();
        string operation = fixture.Enqueue("ashley", Clock);
        fixture.Google.SetRow(fixture.ItemId("ashley"), 93);
        fixture.Google.Header = "Foreign column";
        await fixture.RunSelectedAsync();
        Assert.AreEqual(0, fixture.Google.Mutations.Count);
        Assert.AreEqual(SyncOutboxState.Conflict, fixture.Store.GetSyncOperation(operation).State);
    }
    [DataTestMethod]
    [DataRow("")]
    [DataRow("https://onlyfans.com/123456789/johnny_guides")]
    public async Task ForeignFormulasNeverAuthorizeWritesOrCompletion(string calculated)
    {
        using SyncFixture fixture = SyncFixture.Create();
        string operationId = fixture.Enqueue("ashley", Clock);
        fixture.Google.SetRow(fixture.ItemId("ashley"), 93);
        fixture.Google.SetCell(Range(93), "=\"" + calculated + "\"");
        await fixture.RunSelectedAsync();
        Assert.AreEqual(0, fixture.Google.Mutations.Count);
    ...
    private sealed class FakeGoogleHandler : HttpMessageHandler
    {
        ...
        )
        {
            ...
            if (request.Method == HttpMethod.Get && path.Contains("/values/", StringComparison.Ordinal))
            {
                ...
                if (range.EndsWith("1", StringComparison.Ordinal) && !char.IsDigit(range[^2]))
                    return Json(request, JsonSerializer.Serialize(new { range, values = new[] { new[] { Header } } }));
                CellReads.Add(range);
                _cells.TryGetValue(range, out string? value);
                if (value?.StartsWith("=\"", StringComparison.Ordinal) == true && !request.RequestUri.Query.Contains("FORMULA", StringComparison.Ordinal))
                    value = value[2..^1];
            ...
            if (request.Method == HttpMethod.Post && path.EndsWith("/values:batchUpdateByDataFilter", StringComparison.Ordinal))
            {
                string body = await request.Content!.ReadAsStringAsync(cancellationToken);
                using JsonDocument json = JsonDocument.Parse(body);
                JsonElement data = json.RootElement.GetProperty("data");
                JsonElement update = data[0];
                Assert.AreEqual("RAW", json.RootElement.GetProperty("valueInputOption").GetString());
                Assert.AreEqual(7000, update.GetProperty("dataFilter").GetProperty("developerMetadataLookup").GetProperty("metadataId").GetInt32());
                BeforeMutation?.Invoke();
                var rowValues = update.GetProperty("values")[0];
                Assert.IsTrue(rowValues.EnumerateArray().Take(rowValues.GetArrayLength() - 1).All(cell => cell.ValueKind == JsonValueKind.Null));
                string range = $"'{_sheetTitle}'!{(char)('A' + rowValues.GetArrayLength() - 1)}{_rows[MetadataSearches.Last()][0]}";
                string value = rowValues[rowValues.GetArrayLength() - 1].GetString()!;
