...
var CREATOR_UPLOAD_SPREADSHEET_ID = "1Ninkxbv1SOvatcJ3AP4zwKWxdc32imlIkP_IMUSTR9E";
...
function creatorUploadTokens(value) {
  var values = creatorUploadNormalizedText(value).split(/\s+/).filter(function (token) {
...
function creatorUploadScoreRow(draft, row) {
  ...
  var descriptionScore = creatorUploadSimilarity(draft.description, row.description);
  var targetDay = creatorUploadParseDay(draft.releaseDate);
  var rowDay = creatorUploadParseDay(row.releaseDate);
  var distance = isNaN(targetDay) || isNaN(rowDay)
    ? 99
  ...
  return Math.round(titleScore * 40 + descriptionScore * 14 + dateScore + emptyBonus);
}

function creatorUploadSlugify(value) {
  return creatorUploadNormalizedText(value).replace(/\s+/g, "-") || "untitled-video";
}
...
function creatorUploadCanonicalUrl(platform, value) {
  ...
  if (platform === "onlyfans") {
    match = raw.match(/^(?:https:\/\/onlyfans\.com\/)?(\d+)(?:\/johnny_guides)?\/?$/);
...
function creatorUploadMatchRows(draft, rows) {
  var populated = rows.filter(function (row) { return !creatorUploadEmptyRow(row); });
  var ranked = populated.map(function (row) {
    return {
      row: row,
  ...
  var ids = populated.map(function (row) { return creatorUploadClean(row.id); });
...
function creatorUploadPlanCommit(row, request) {
  if (creatorUploadFingerprint(row) !== creatorUploadClean(request.fingerprint)) {
    return { status: "stale", row: row, fingerprint: creatorUploadFingerprint(row) };
  }
  ...
  if (creatorUploadEmptyRow(row)) {
    ...
    (creatorUploadClean(row.id) !== creatorUploadClean(metadata.id) ||
      creatorUploadClean(row.releaseDate) !== creatorUploadClean(metadata.releaseDate) ||
      creatorUploadClean(row.title) !== creatorUploadClean(metadata.title) ||
      creatorUploadClean(row.description) !== creatorUploadClean(metadata.description))
  ) {
    return { status: "stale", row: row, fingerprint: creatorUploadFingerprint(row) };
  }
  var current = creatorUploadClean(row[field]);
  if (current && current !== postUrl) {
    return { status: "conflict", row: row, fingerprint: creatorUploadFingerprint(row) };
  }
  next[field] = postUrl;
  var status = current === postUrl ? "idempotent" : "updated";
  return { status: status, row: next, fingerprint: creatorUploadFingerprint(next) };
}
...
function creatorUploadHandle(action, payload) {
  ...
  if (action === "matchCatalogue") {
    return creatorUploadMatchRows(creatorUploadValidateDraft(payload || {}), creatorUploadReadRows(sheet));
  }
  if (action !== "commitPlatformLink") throw new Error("Unsupported catalogue action.");
  var lock = LockService.getScriptLock();
  if (!lock.tryLock(15000)) throw new Error("The catalogue is busy; retry this platform.");
  try {
    ...
    var current = creatorUploadReadRows(sheet).find(function (row) { return row.row === rowNumber; });
    ...
    var verified = creatorUploadReadRows(sheet).find(function (row) { return row.row === rowNumber; });
    return {
      status: "updated",
      row: verified,
...
function doPost(event) {
  try {
    var contents = event && event.postData ? creatorUploadClean(event.postData.contents) : "";
    if (!contents || contents.length > 32000) throw new Error("Invalid request body.");
    var request = JSON.parse(contents);
    var expected = PropertiesService.getScriptProperties().getProperty(CREATOR_UPLOAD_SECRET_PROPERTY);
    if (!expected || creatorUploadClean(request.secret) !== expected) {
      throw new Error("Catalogue bridge authorization failed.");
    }
    return creatorUploadJson({ ok: true, result: creatorUploadHandle(request.action, request.payload || {}) });
  } catch (error) {
    return creatorUploadJson({ ok: false, error: creatorUploadClean(error.message) || "Catalogue bridge failed." });
  }
}
