...
This is listing input, not a claim of store approval. Build with
`npm run build:store` and submit only that generated store ZIP. The personal
extension's broader site actions, remote avatars, debugger, and native hosts do
not belong in this product. Validate the package against the declared single
purpose and test affirmative consent, local data handling, and disabling/reset.

The privacy URL above serves `docs/privacy.html`, also the packaged policy; do not
maintain another hand-edited policy copy. Confirm the published page and dashboard
declarations match the candidate before submission. The icon source is
`extensions/store/icons/icon128.png`; `brand-master.png` and the SVG files here are
editable marketing masters, while the promo/settings PNGs are listing assets,
not evidence of a current test run. Regenerate a screenshot deliberately when
the shipped interface changes; automated tests write captures only to test output.

Display-only behavior does not itself guarantee compatibility with platform
terms, privacy obligations, or store review. Obtain current platform and
distribution-policy review before public release rather than treating old policy
summaries as approval. No review outcome is implied by this repository.
