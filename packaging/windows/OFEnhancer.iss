...
OutputBaseFilename=OFEnhancer-Setup-0.20.59
...
begin
  ...
  Parameters := Operation + ' --install-root "' + WizardDirValue + '" --package-version 0.20.59 --error-file "' + ErrorFile + '" --resume-root-file "' + ExpandConstant('{tmp}\ofenhancer-resume-root.txt') + '"';
...
begin
  if (CurStep = ssPostInstall) and IsFreshReset() then
    if not Exec(ExpandConstant('{app}\desktop\OFEnhancer.Desktop.exe'),
      '--fresh-reinstall-installed --install-root "' + ExpandConstant('{app}') + '" --package-version 0.20.59',
