...
Release 0.20.19 uses Node 24.19.0, the locked npm dependencies, .NET SDK 8.0.425/runtime 8.0.31, and SQLitePCLRaw.lib.e_sqlite3 2.1.13. Current npm and transitive NuGet audits were clean on September 14, 2026. The native SQLite regression executes `sqlite_version()` and requires the aggregate-memory-corruption fix (3.50.2 or later). .NET 8 is in maintenance and reaches end of support November 10, 2026; migrate to a supported major before then. Verify the actual self-contained runtime and native library again in the release stage.
