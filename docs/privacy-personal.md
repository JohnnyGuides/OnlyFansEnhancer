...
keeping it and offers explicit protected cleanup of the configured OFEnhancer
data and WebView2 roots; silent uninstall keeps them. External selected media/audit roots, downloaded credential
JSON, Google spreadsheets, and Chrome profiles are not erased by that choice.
Fresh reinstall is a separate clean-slate operation: Windows first removes safely identified application-owned settings, catalogue,
local Google token/client files, checkpoints, WebView2 state, caches and obsolete
program files. The installed app then keeps a separate Chrome replacement task;
Windows completion does not claim that the prior extension was removed. It does not remove source media, external credential JSON, cloud
